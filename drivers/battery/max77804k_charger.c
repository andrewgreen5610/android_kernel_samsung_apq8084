		...
		if (val->intval == POWER_SUPPLY_TYPE_BATTERY) {
			...
			if (force_fast_charge == FAST_CHARGE_FORCE_AC) {/* We are in basic Fast Charge mode, so we substitute AC to USB levels */
				switch(charger->cable_type) {
					/* These are low current USB connections, apply usual 1A/h AC levels to USB */
			...
			/* We are in custom current Fast Charge mode for both AC and USB */
				...
				switch(charger->cable_type) {
					...
					case POWER_SUPPLY_TYPE_OTG:
									...
									break;
					/* These are AC connections, apply custom AC current for all of them */
